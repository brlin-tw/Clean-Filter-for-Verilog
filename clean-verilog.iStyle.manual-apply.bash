...
# Clean filter for Verilog using iStyle(https://github.com/thomasrussellmurphy/istyle-verilog-formatter) wrapper for manual applying
# 林博仁 © 2017
...
main() {
	if [ $PROGRAM_ARGUMENT_ORIGINAL_NUMBER -ne 1 ]; then
		...
		local target_file="$PROGRAM_ARGUMENT_ORIGINAL_LIST"
		...
		"${filter}" <"${target_file}" >"$temp_file" 2>/dev/null
